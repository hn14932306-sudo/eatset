...
完整步驟（啟用 API、限制金鑰、驗證歸因）：見 **[docs/places-setup.md](docs/places-setup.md)**；本機模擬器見 **[docs/local-android-run.md](docs/local-android-run.md)**。
...
Nearby 回傳後由 `DecisionEngine` 偏好營業中、較高評分店家（見 [docs/places-setup.md](docs/places-setup.md)；本機模擬器見 [docs/local-android-run.md](docs/local-android-run.md)）。
